...
Когда **ModelBase** просматривает словарь **dict** атрибутов, которые составят класс модели, который он создает, он проверяет каждый атрибут, который в конечном итоге окажется в модели, чтобы увидеть, _**имеет ли значение, присвоенное атрибуту, метод с именем**_ `serve_to_class()`. Если это так, он вызовет метод `contribute_to_class()`, передав определяемый класс модели и имя назначаемого атрибута. Это позволяет перенести большую часть работы из **ModelBase** в различные типы вещей, которые в конечном итоге становятся атрибутами в классах моделей.

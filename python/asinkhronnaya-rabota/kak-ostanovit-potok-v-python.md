...
Мы можем хотеть спать часами.

Может быть полезно выполнять действие в приложении каждый час или два раза в день.

Этого можно добиться, указав количество часов сна в секундах.

Например:

* 1 час это 3600 секунд
* 10 часов это 36000 секунд
* 100 часов — это 360 000 секунд

Возможно, интервал сна будет проще указать в вашей программе как умножение. Например, 2 часа 30 минут можно указать как `2,5*(60*60)`, для наглядности добавлены скобки.
...
Пример ниже демонстрирует, как спать 12 часов.
...
Мы можем обновить программу, чтобы она запускалась один раз каждые 12 часов или два раза в день, вызывая сон в цикле.

Например:

```python
# SuperFastPython.com
# пример запуска два раза в день
from time import sleep
# запустить навсегда
while True:
    # сделать задачу
...
Нам может понадобиться, чтобы наш поток спал несколько дней.

Может быть полезно выполнять действие один раз в день или один раз в неделю.

Этого можно добиться, указав количество дней сна в секундах.

Например:

* 1 день — 86 400 секунд.
* 10 дней — это 864 000 секунд.
* 100 дней — это 8 640 000 секунд.

Возможно, интервал будет проще указать в вашей программе как умножение. Например, 2 дня и 12 часов можно указать как `2,5 * (60 * 60 * 24)`, для наглядности добавлены скобки.

* 1 час: `1*(60*60*24)`
* 10 часов: `10*(60*60*24)`
* 100 часов: `100*(60*60*24)`

Пример ниже демонстрирует, как спать в течение 7 дней.

```python
# SuperFastPython.com
# пример сна в течение нескольких дней
from time import sleep
# вывод сообщения
...
Мы можем обновить программу, чтобы она запускалась раз в неделю, вызывая сон в цикле.

Например:

```python
# SuperFastPython.com
# пример запуска один раз в неделю
from time import sleep
# запустить навсегда
while True:
    # сделать задачу
...
Далее давайте рассмотрим несколько советов по использованию сна в вашей программе.
...
В этом разделе приведены некоторые моменты использования функции time.sleep() в вашей программе.

Они есть:

1. Режим сна не снимает приобретенные блокировки.
2. Используйте сон в цикле ожидания занятости.
3. Вы можете спать ноль секунд.
4. Подумайте о том, чтобы подождать вместо того, чтобы спать.
5. Защититесь от SIGINT.

Давайте подробнее рассмотрим каждый по очереди.

### 5.1. Сон не снимает блокировки

В конкурентном программировании мы можем получить доступ к критическим разделам кода, сначала установив блокировку, например блокировку взаимного исключения (mutex).

Мы также можем получить условие, которое получит внутреннюю блокировку, или получить семафор, который изменит внутренний счетчик, защищенный блокировкой.

Если мы вызовем time.sleep(), удерживая блокировку, ресурс не освободится.
...
Сон, удерживая блокировку, не позволит любому другому потоку получить блокировку. Проблема в том, что это может привести к длительным задержкам в вашей программе.

Вам следует избегать этого, если только это не является особым требованием вашей программы.

В идеале вы должны снять все блокировки перед переводом потока в спящий режим.
...
with lock:
	# что-то делать
	...
# спать какой-то время
sleep(60)
```

### 5.2. Используйте сон, когда заняты ожиданием

Ожидание занятости, также называемое spinning, относится к потоку, который неоднократно проверяет условие в цикле.

Это форма ожидания, которая вызывает непрерывную проверку желаемого состояния, а не блокирует и разрешает выполнение других потоков.

Мы можем использовать ожидание занятости при попытке скоординировать два потока на основе времени, чтобы не пропустить событие или изменение состояния.

Вы можете узнать больше о занятом ожидании в этом уроке:

* [Как использовать ожидание занятости в Python](https://superfastpython.com/thread-busy-waiting-in-python/)

Спин-блокировка — это особый тип ожидания занятости, используемый для ожидания блокировки взаимного исключения.

Вы можете узнать больше о спин-блокировках в этом уроке:

* [Как использовать спин-блокировку в Python](https://superfastpython.com/thread-spinlock-in-python/)

Например, следующий цикл демонстрирует спин-блокировку, при которой цикл ожидания занятости пытается получить блокировку мьютекса на каждой итерации.

```python
...
# пример занятого цикла ожидания спин-блокировки
while True:
    # попробуй получить блокировку
    acquired = lock.acquire(blocking=False)
    # проверить, получили ли мы блокировку
    if acquired:
        # остановить ожидание
        break
```

Вычислительную нагрузку, связанную с занятым циклом ожидания, можно уменьшить, добавив режим ожидания.

Это задержит проверку желаемого состояния на каждой итерации цикла, но заставит поток заблокироваться и разрешить выполнение других потоков, что, возможно, улучшит общую производительность.

В примере ниже показано добавление задержки с помощью вызова time.sleep() к спин-блокировке.

Например:

```python
...
# пример занятого цикла ожидания для спин-блокировки со спящим режимом
while True:
    # попробуй получить блокировку
    acquired = lock.acquire(blocking=False)
    # проверить, получили ли мы блокировку
